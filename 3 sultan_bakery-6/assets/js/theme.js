(function($) {
    ...
    function fun_name() {
      ...
      $(document).off('click', 'a[data-title=Fullscreen');
      $(document).on('click' , 'a[data-title=Fullscreen]', fun_name);

    $(document).on('click', '#header a[href="account.general.html"]', function(e) {
        var $active = $('[data-nav]').find('.active');
        $active.children('a').addClass('animation-icon').find('img').eq(1).removeClass('d-none');
        $active.removeClass('active');
    });

    function aside_activate(self) {
        var $this = $(self),
            $active, $li, $li_li;
        ...
        if ($this.has('img')) {
            $this.removeClass('animation-icon').find('img').eq(1).addClass('d-none');
            $active.not($li_li).not($li).children('a').addClass('animation-icon').find('img').eq(1).removeClass('d-none');
        }
    ...
    $(document).on('click', '[data-nav] a', function(e) {
        let self = this;
        if ($(this).attr('href') && $(this).attr('href') != '#' && $('footer .save').length && $('footer').is(':visible') && !$('footer .save').is('[disabled]')) {
    ...
    function content_aside_activate(self) {
        var $this = $(self),
            $active, $div;

        $div = $this.parent();

        $active = $div.find('.active');

        $this.addClass('active').removeClass('animation-icon').find('img').eq(1).addClass('d-none');
        $active.not($this).removeClass('active').addClass('animation-icon').find('img').eq(1).removeClass('d-none');
    ...
    $(document).on('click', '[data-sub-nav] a', function(e) {
        let self = this;
        if ($(this).attr('href') && $(this).attr('href') != '#' && $('footer .save').length && $('footer').is(':visible') && !$('footer .save').is('[disabled]')) {
            $('#page-leave-modal .leave').click(function() {
                content_aside_activate(self);
            })
        } else {
            content_aside_activate(self);
        }
